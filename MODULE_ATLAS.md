...
Modules: 121 | Functions: 955 | Lines: 92,000
...
**Role:** rendering/shells | **Lines:** 891
...
**Role:** rendering/shells | **Lines:** 1,086
...
- `calculate_planet9_position_on_orbit(a, e, i, omega, Omega, theta)` (line 213) -- Calculate position that lies exactly on the orbit defined by the parameters
- `rotate_points2(x, y, z, angle, axis)` (line 261) -- Rotates points (x,y,z) about the given axis by 'angle' radians.
- `calculate_axis_range(objects_to_plot)` (line 309) -- Calculate appropriate axis range based on outermost planet
- `fetch_trajectory(object_id, dates_list, center_id, id_type, start_date, end_date)` (line 326) -- Fetch trajectory data in batch for all dates, handling missing epochs through interpolation.
- `fetch_orbit_path(obj_info, start_date, end_date, interval, center_id, id_type)` (line 552) -- Fetch orbit path data from JPL Horizons for the given object between start_date and end_date,
- `pad_trajectory(global_dates, object_start_date, object_end_date, object_id, center_id, id_type)` (line 602) -- Fetch trajectory and pad with None before start_date and after end_date.
- `add_url_buttons(fig, objects_to_plot, selected_objects)` (line 637) -- Add URL buttons for missions and objects in solar system visualizations.
- `get_default_camera()` (line 747) -- Return the default orthographic camera settings for top-down view
- `print_planet_positions(positions)` (line 759) -- Print positions and distances for planets.
- `create_orbit_backup()` (line 789) -- Create a backup of orbit cache on startup
- `cleanup_old_orbits()` (line 822) -- Remove orbit data older than 30 days
- `show_animation_safely(fig, default_name)` (line 894) -- Show and optionally save an animation.
...
- `classify_role(module_name)` (line 192) -- Classify a module's functional role.
- `get_module_docstring(filepath)` (line 245) -- Extract module-level docstring, falling back to leading comments.
- `get_public_functions(filepath)` (line 332) -- Extract public function/class names with their docstrings.
- `get_local_imports(filepath, local_modules)` (line 363) -- Extract project-local imports from a Python file.
- `count_lines(filepath)` (line 386) -- Count non-blank lines in a file.
- `build_dependency_graph(project_dir)` (line 399) -- Build bidirectional dependency graph for all local modules.
- `scan_modules(project_dir)` (line 424) -- Scan every .py file in project_dir once and return the list of
- `generate_atlas(modules, output_path)` (line 466) -- Write MODULE_ATLAS.md from an already-scanned modules list
- `generate_index(modules, output_path)` (line 635) -- Write MODULE_INDEX.md from the SAME already-scanned modules list
- `main()` (line 708)
...
- `action_tier(score)` (line 294) -- Return tier number (1=highest priority, 4=lowest).
- `classify_domain(module_name)` (line 460) -- Classify a module's report domain. Returns (domain, was_mapped).
- `has_citation(text, is_docstring)` (line 528) -- Does the given text block contain a citation marker?
- `has_stale_marker(text)` (line 543) -- Does the given text contain a staleness indicator?
- `extract_numeric_claims(text)` (line 571) -- Yield (num_str, unit, value_float) for each numeric claim in text.
- `build_name_import_map(project_dir, local_modules)` (line 591) -- For each local module, find which NAMES other modules import from it.
- `name_is_imported(name, module_name, imported_names)` (line 629) -- Return (count, consumers) for a name defined in module_name.
- `class ProvenanceUnit` (line 642) -- The smallest thing that has a coherent source citation.
- `get_context_block(lines, unit_start_line, unit_end_line, lookback, lookahead)` (line 707) -- Return the block of text a unit can see for citation purposes.
- `get_unit_interior(lines, line_start, line_end)` (line 728) -- Return the text inside the unit itself (per-entry comments).
- `extract_numeric_value(node)` (line 746) -- Evaluate an AST node to a numeric constant.
- `extract_units_from_file(filepath, module_name, role)` (line 769) -- Walk the AST of one file and emit ProvenanceUnits.
- `build_pinned_values(project_dir)` (line 1021) -- Extract numeric values from constants_new.py that have source citations.
- `score_unit(unit, imported_names, pinned_values)` (line 1077) -- Assign vulnerability and criticality to a unit.
- `canonical_concept(name)` (line 1177) -- Map a constant name to its canonical concept, or None.
- `find_cross_file_issues(units)` (line 1188) -- Find same-concept constants across multiple files.
- `load_exceptions(project_dir)` (line 1232) -- Load provenance_exceptions.json from data/ subdirectory if present.
- `is_suppressed(unit, suppressed_fingerprints)` (line 1264) -- Check if a unit matches any suppressed fingerprint.
- `format_accepted_residuals(accepted_residuals)` (line 1293) -- Format accepted residuals as a markdown block for the audit report.
- `scan_project(project_dir, output_path)` (line 1347) -- Scan all .py files and produce the provenance audit report.
- `generate_report(units, consistent_dups, inconsistencies, files_scanned, project_dir, output_path, accepted_residuals, coverage_gaps)` (line 1409) -- Write PROVENANCE_AUDIT.md.
- `main()` (line 1749)
...
- `human(n)` (line 30) -- Format byte count as human-readable string.
- `inventory(dirs, skip_hidden)` (line 39) -- Walk dirs and group files by extension. Returns dict of extension stats.
- `gallery_inventory(gallery_path)` (line 69) -- Walk the gallery repo and return extension stats + total size.
- `peek_orbit(path)` (line 102) -- Peek at orbit_paths.json structure.
- `peek_pickle(path)` (line 122) -- Peek at pickle file structure.
- `peek_gallery_metadata(gallery_path)` (line 135) -- Peek at gallery_metadata.json: entry count, fields, categories.
- `write_ext_table(out, stats)` (line 170) -- Write the extension breakdown table.
- `main()` (line 180)
...
- `resolve_center_slug(center_body)` (line 212) -- Map a Horizons center (@-id or name) to a schema slug. Rejects unmapped.
- `parse_osc_epoch_to_jd(epoch_str)` (line 276) -- Osculating epoch string -> JD. Strips the trailing ' osc.' and parses the
- `run_preflight()` (line 330) -- Step 0 (0a-0f) + Step 0-STOP. Read-only. Prints the ground truth.
- `build_osculating_entry(osc_cache, obj, warn)` (line 418)
- `write_position_file(cache, obj, out_dir, warn)` (line 470)
- `assert_invariants(index, out_dir)` (line 552)
- `write_feature_configs(out_dir, warn)` (line 577) -- Step 6: best-effort feature params from SHELL_CONFIGS (pulls plotly).
- `run_export(output_dir, full_catalog)` (line 600)
- `main(argv)` (line 680)
...
- `parse_rice(rice_str)` (line 233) -- Parse a rice:R/I/C/E string into (reach, impact, confidence, effort) or None.
- `compute_score(rice_tuple)` (line 254) -- Compute RICE score: R x I x (C / 100) / E.  Returns float or None.
- `fmt_score(score)` (line 262) -- Format a RICE score for the index table.
- `parse_blocks(lines)` (line 269) -- Return list of dicts (one per L-block) and a list of parse problems.
- `find_span(lines, start_prefix, closing_re)` (line 293) -- Find the physical span of a heading starting with start_prefix: from
- `find_top_level_span(lines, start_prefix)` (line 334)
- `find_subsection_span(lines, start_prefix)` (line 338)
- `find_c_bounds(lines)` (line 342) -- Return (c_open_idx, d_idx) for the general C archive's physical span.
- `find_bucket_destination_span(lines, target_tag)` (line 347) -- Return the (start, end) span that is the ACTUAL destination for a
- `find_closed_bucket_issues(blocks, lines)` (line 364) -- Single source of truth for every closed-bucket problem -- consumed by
- `check(blocks, problems, lines)` (line 474) -- Structural + status/section consistency checks. `lines` is optional
- `cell(s)` (line 496) -- Make a string safe for a markdown table cell (escape the pipe).
- `fmt_disposition(b)` (line 501)
- `fmt_id(b)` (line 506)
- `build_index(blocks)` (line 511)
- `extract_block_text(lines, start_line)` (line 570) -- Extract the full text of one L-block starting at start_line (the #### header).
- `reconcile_closed_buckets(text, blocks, lines)` (line 592) -- Apply every AUTO-FIXABLE issue from find_closed_bucket_issues -- the
- `main()` (line 701)
...
> Transactional patch: document color/RGB exclusion from provenance-scanner claims, project-wide, per Tony's call (July 16, 2026). Run this once from the repo root (same directory as provenance_scanner.py). Verified against a disposable clone: py_compile clean, ASCII-only, and re-running the scanne...
...
**Consumed by:** export_orbit_cache, palomas_orrery, planet_visualization, smoke_dipole_cone, smoke_rotation_axis
...
| export_orbit_cache | other | 611 | 3 | 0 |
...
| jupiter_visualization_shells | rendering/shells | 891 | 3 | 1 |
| ledger_index | other | 662 | 0 | 0 |
...
| palomas_orrery_helpers | utility | 769 | 10 | 2 |
...
| provenance_scanner | devtool | 1,553 | 1 | 0 |
| provenance_scanner_color_patch | other | 88 | 0 | 0 |
| report_manager | utility | 124 | 0 | 4 |
| saturn_visualization_shells | rendering/shells | 1,086 | 3 | 2 |

...
**Role:** gui | **Lines:** 9,490

> palomas_orrery.py - Main GUI and plotting engine for Paloma's Orrery.

**Depends on:** apsidal_markers, asteroid_belt_visualization_shells, celestial_objects, close_approach_data, comet_visualization_shells, constants_new, earth_system_visualization_gui, exoplanet_orbits, exoplanet_stellar_properties, exoplanet_systems, formatting_utils, idealized_orbits, info_dictionary, orbit_data_manager, orbital_elements, orbital_param_viz, osculating_cache_manager, palomas_orrery_helpers, planet_visualization, save_utils, sgr_a_grand_tour, shared_utilities, shell_configs, shutdown_handler, social_media_export, solar_visualization_shells, spacecraft_encounters, star_sphere_builder, visualization_utils
**Consumed by:** (none -- standalone)

**Public functions:**

- `get_fetch_interval_for_type(obj_type, obj_name, trajectory_interval, default_interval, satellite_interval, planetary_params)` (line 317) -- Get the appropriate fetch interval based on object type.
- `create_dates_list_for_object(obj, obj_type, date_obj, trajectory_points, orbital_points, satellite_days, satellite_points, start_date, end_date, planetary_params, parent_planets, center_object_name, max_date, settings)` (line 338) -- Create a list of dates for plotting based on object type.
- `handle_update_dialog(num_objects)` (line 420) -- Handle the update dialog for cache updates.
- `get_interval_settings()` (line 482) -- Get all interval settings from the GUI entries.
- `get_date_from_gui()` (line 556) -- Get the date from GUI entry fields.
- `create_animation_dates(current_date, step, N)` (line 575) -- Create dates list specifically for animations.
- `calculate_axis_range_from_orbits(selected_objects, positions, planetary_params, parent_planets, center_object_name)` (line 610) -- Calculate appropriate axis range based on orbital parameters.
- `get_improved_axis_range(scale_var, custom_scale_entry, selected_objects, positions, planetary_params, parent_planets, center_object_name)` (line 832) -- Get axis range using improved scaling logic.
- `get_animation_axis_range(scale_var, custom_scale_entry, objects, planetary_params, parent_planets, center_object_name)` (line 850) -- Get axis range for animations using the same logic as static plots.
- `calculate_satellite_precession_info(selected_objects, start_date, end_date, center_object_name)` (line 871) -- Calculate precession information for selected satellites based on date range.
- `load_window_config()` (line 1030) -- Load saved window geometry and sash positions from config file.
- `save_window_config()` (line 1040) -- Save current window geometry and sash positions to config file.
- `fetch_position(object_id, date_obj, center_id, id_type, override_location, mission_url, mission_info)` (line 1161)
- `calculate_analytical_position(obj_name, date_obj, center_id)` (line 1637) -- Calculate position from analytical orbital elements when Horizons is unavailable.
- `fetch_radec_for_hover(object_id, date_obj, id_type)` (line 1720) -- Fetch RA/Dec and uncertainties for hover text
- `add_celestial_object(fig, obj_data, name, color, symbol, marker_size, hover_data, center_object_name, params, show_legend)` (line 1790)
- `get_planet_shell_vars_map()` (line 1874) -- (2d) Single source for the planet -> shell-vars mapping.
- `resolve_shell_sun_position(center_object_name, sun_entry, plot_date, center_id)` (line 1900) -- (2b) Resolve the Sun position used to orient magnetosphere/bow-shock
- `add_center_body_marker(fig, center_object_name, params)` (line 1931) -- (2a) Canonical center-body marker -- the ONE mechanism, both pipelines.
- `add_center_body_shells(fig, center_object_name, sun_shell_vars_map, planet_shell_vars_map, sun_position, scale_value, axis_range, log_prefix, skip_elements, animate)` (line 1968) -- (2c) Center-body shell dispatch -- one implementation, both pipelines.
- `get_center_engine_elements(center_object_name)` (line 2164) -- (C6d fix) The set of CENTER-body element names the per-frame engine
- `collect_perframe_elements(center_object_name, candidate_body_names, center_shell_radius_au)` (line 2230) -- (Phase 3) Element specs for engine-animatable primitives.
- `build_perframe_traces(spec, body_pos, sun_pos, frame_entry, frame_date, quiet)` (line 2393) -- (Phase 3) The one rebuild mechanism: call the spec's builder at this
- `allocate_perframe_elements(fig, specs, positions_over_time, engine_sun_traj, dates_list)` (line 2449) -- (Phase 3) Frame-1 allocation: build each element once at its frame-1
- `add_static_only_legend_placeholders(fig, center_object_name)` (line 2541) -- (Phase 3) Greyed-out legend disclosure: every shell CHECKED for a body
- `update_status_display(message, status_type)` (line 2607) -- Update status display with color coding and history
- `configure_controls_canvas(event)` (line 2646)
- `class ScrollableFrame` (line 3429) -- A scrollable frame that can contain multiple widgets with a vertical scrollbar.
- `class CreateToolTip` (line 3530) -- Create a tooltip for a given widget with intelligent positioning to prevent clipping.
- `pulse_progress_bar()` (line 3687) -- Create a pulsating effect for the progress bar
- `update_orbit_paths(center_object_name)` (line 3693) -- For each object in the global 'objects' list that has an 'id', check if its orbit path is
- `plot_orbit_paths(fig, objects_to_plot, center_object_name)` (line 3791) -- Plot orbit paths using data from orbit_data_manager or temp cache.
- `plot_actual_orbits(fig, planets_to_plot, dates_lists, center_id, show_lines, center_object_name, show_closest_approach, trajectory_marker_color)` (line 3864) -- Plot actual orbit positions for selected objects.
- `export_social_view()` (line 4324) -- Export the last plotted figure as a social media view.
- `plot_objects()` (line 4351)
- `animate_objects(step, label)` (line 6113)
- `on_closing()` (line 8170) -- Handle cleanup when the main window is closed.
- `periodic_config_save()` (line 8195)
- `set_palomas_birthday()` (line 8202)
- `update_date_fields(new_date)` (line 8206)
- `fill_now()` (line 8219)
- `reset_all_selections()` (line 8246) -- Return the GUI to its STARTUP state, behind a confirm dialog.
- `calculate_next_vernal_equinox(from_date)` (line 8347) -- Calculate the next vernal equinox (March equinox) from a given date.
- `fill_next_vernal_equinox()` (line 8417) -- Fill the date fields with the next vernal equinox from the current date.
- `toggle_all_shells()` (line 8428) -- Toggle all sun shell checkboxes based on the main shell checkbox.
- `handle_mission_selection()` (line 8460)
- `animate_one_minute()` (line 8466)
- `animate_one_hour()` (line 8472)
- `animate_one_day()` (line 8477)
- `animate_one_week()` (line 8483)
- `animate_one_month()` (line 8488)
- `animate_one_year()` (line 8493)
- `animate_palomas_birthday()` (line 8498)
- `report_callback_exception(exc_type, exc_value, exc_traceback)` (line 8528)
- `sync_end_date_from_days()` (line 8540) -- Calculate end date from start date + days to plot
- `sync_days_from_dates()` (line 8571) -- Calculate days to plot from start and end dates
- `sync_days_from_dates()` (line 8583) -- Calculate days to plot from start and end dates
- `sync_end_date_from_days()` (line 8686) -- Calculate end date from start date + days to plot
- `sync_days_from_dates()` (line 8718) -- Calculate days to plot from start and end dates
- `get_end_date_from_gui()` (line 8730) -- Get end date from GUI fields. Defaults empty fields to avoid crash.
- `can_be_horizons_center(obj)` (line 8762) -- Check if object can be used as Horizons coordinate center.
- `create_celestial_checkbutton(name, variable)` (line 8864)
- `create_mission_checkbutton(name, variable, dates)` (line 9385)
- `create_comet_checkbutton(name, variable, dates, perihelion)` (line 9829) -- Creates a checkbutton for a comet with a tooltip containing its description,
- `create_interstellar_checkbutton(name, variable, dates, perihelion)` (line 9947) -- Creates a checkbutton for an interstellar/hyperbolic object with a tooltip
- `toggle_special_fetch_mode()` (line 10044) -- DEPRECATED: Special fetch mode removed - two-layer trajectories provide automatic detail
- `create_exoplanet_checkbutton(name, variable, is_star)` (line 10060) -- Create checkbutton for exoplanet objects
- `open_star_visualization()` (line 10081) -- Inform user about standalone Star Visualization executable.
- `launch_galactic_center()` (line 10154) -- Launch the Sagittarius A* Grand Tour visualization.
- `update_center_dropdown()` (line 10340) -- Update the center dropdown to show only Sun + selected centerable objects.
- `setup_center_dropdown_traces()` (line 10408) -- Add traces to all object IntVars to update center dropdown on selection change.
- `on_center_change()` (line 10423) -- Update frame title and status when the center object is changed.
- `open_orbital_param_visualization()` (line 10917) -- Opens the orbital parameter visualization window by calling the
- `restore_sash_positions()` (line 11089)
...
- `create_celestial_body_visualization(fig, body_name, shell_vars, animate, frames, center_position, sun_position, object_type, center_object, skip_elements, axis_range)` (line 342) -- Unified config-driven dispatch for celestial body shell visualization.
- `create_planet_visualization(fig, planet_name, shell_vars, animate, frames, center_position, sun_position)` (line 558) -- RETIRED (June 2026, animation refactor Phase 2.5 / D.Structural 3).
- `create_planet_shell_traces(planet_name, shell_vars, center_position)` (line 752) -- Creates traces for planet shells without adding them to a figure.
...
- `create_magnetosphere_shape(params, n_phi, n_theta, n_tail_segments)` (line 208) -- Creates points for a magnetosphere with asymmetry, compressed on sunward side
- `create_bow_shock_shape(standoff, width, n_phi, n_theta, eccentricity)` (line 298) -- Generate body-frame point cloud for a bow shock surface of revolution.
...
- `add_hover_toggle_buttons(fig)` (line 29) -- Add hover text toggle buttons to any Plotly figure.
- `add_camera_center_button(fig, center_object_name)` (line 88) -- Add a button to move the camera to the center object.
- `add_look_at_object_buttons(fig, positions, center_object_name, target_objects, show_target_marker)` (line 163) -- Add buttons to point camera from center toward specific target objects.
- `build_scene_axis(axis_range, axis_title, dtick, autorange, show_grid, show_background)` (line 376) -- Build one 3D scene axis dict (x, y, or z) for an orrery plot.
- `build_scene_axes(axis_range, dtick, autorange, show_grid, show_background)` (line 418) -- Build the x/y/z axis dicts for a 3D orrery scene.
- `build_scene(axis_range, camera, domain, dtick, autorange, auto_dtick, aspectmode, show_grid, show_background)` (line 439) -- Build the full 3D scene dict for an orrery plot: x/y/z axes plus
- `add_fly_to_object_buttons(fig, positions, center_object_name, target_objects, fly_distance, distance_scale_factor, target_extents)` (line 494) -- Add buttons to fly the camera TO specific target objects, keeping focus on the object.
- `format_hover_text(obj_data, name, is_solar_system)` (line 705) -- Format hover text consistently for different types of objects.
- `format_detailed_hover_text(obj_data, obj_name, center_object_name, objects, planetary_params, parent_planets, CENTER_BODY_RADII, KM_PER_AU, LIGHT_MINUTES_PER_AU, KNOWN_ORBITAL_PERIODS)` (line 764) -- Generate detailed hover text for celestial objects with comprehensive information.
- `update_figure_frames(fig, include_hover_toggle)` (line 987) -- Update figure frames to maintain hover text toggle functionality in animations.
...
**Role:** rendering/shells | **Lines:** 1,850
...
- `calculate_tail_activity_factor(current_distance_au, perihelion_distance_au, max_active_distance_au)` (line 405) -- Calculate how active the comet is based on solar distance.
- `create_comet_nucleus(center_position, nucleus_size_km, comet_name)` (line 441) -- Creates a comet nucleus visualization as a single point.
- `create_maps_disintegration_marker(position_au, comet_name)` (line 487)
- `create_maps_ghost_tail_trace(fig)` (line 580) -- Ghost tail arc for MAPS C/2026 A1, overlaid on the perihelion
- `create_comet_coma(center_position, coma_radius_km, activity_factor, comet_name)` (line 736) -- Creates the coma (atmosphere) around the nucleus.
- `create_comet_dust_tail(center_position, velocity_vector, max_tail_length_mkm, activity_factor, comet_name, num_particles, sun_relative_position)` (line 830) -- Creates the dust tail (Type II tail).
- `create_comet_ion_tail(center_position, max_tail_length_mkm, activity_factor, comet_name, num_particles, sun_relative_position)` (line 1020) -- Creates the ion tail (Type I tail, plasma tail).
- `create_comet_anti_tail(center_position, anti_tail_length_km, activity_factor, comet_name, anti_tail_color, collimation_ratio, num_particles, sun_relative_position)` (line 1170) -- Creates anti-tail jet structure pointing TOWARD the Sun.
- `create_complete_comet_visualization(comet_name, center_position, velocity_vector, current_distance_au)` (line 1374) -- Creates a complete comet visualization with nucleus, coma, and both tails.
- `add_comet_tails_to_figure(fig, comet_name, position_data, center_object_name, current_date, sun_position)` (line 1602) -- Add comet visualization to figure with feature-specific thresholds.
- `build_comet_tail_traces(comet_name, position_data, center_object_name, current_date, sun_position)` (line 2090) -- Trace-returning core for comet tail visualization (21/51 Phase 3
...
- `create_earth_inner_core_shell(center_position)` (line 59) -- Creates Earth's inner core shell.
- `create_earth_outer_core_shell(center_position)` (line 131) -- Creates Earth's outer core shell.
- `create_earth_lower_mantle_shell(center_position)` (line 202) -- Creates Earth's lower mantle shell.
- `create_earth_upper_mantle_shell(center_position)` (line 272) -- Creates Earth's upper mantle shell.
- `create_earth_crust_shell(center_position)` (line 342) -- Creates Earth's crust shell using Mesh3d for better performance with improved hover.
...
**Role:** rendering/shells | **Lines:** 464
...
- `create_eris_core_shell(center_position)` (line 44) -- Creates Eris's core shell.
- `create_eris_mantle_shell(center_position)` (line 140) -- Creates Eris's mantle shell.
- `create_eris_crust_shell(center_position)` (line 216) -- Creates eris's cloud layer shell.
- `create_eris_atmosphere_shell(center_position)` (line 374) -- Creates eris's atmosphere shell.
- `create_eris_hill_sphere_shell(center_position)` (line 467) -- Creates Eris's Hill sphere shell.
...
**Role:** rendering/shells | **Lines:** 543
...
- `create_moon_inner_core_shell(center_position)` (line 42) -- Creates the Moon's inner core shell.
- `create_moon_outer_core_shell(center_position)` (line 110) -- Creates the Moon's outer core shell.
- `create_moon_mantle_shell(center_position)` (line 217) -- Creates the Moon's lower mantle shell.
- `create_moon_crust_shell(center_position)` (line 308) -- Creates Earth's crust shell using Mesh3d for better performance with improved hover.
- `create_moon_exosphere_shell(center_position)` (line 485) -- Creates the Moon's exosphere shell.
- `create_moon_hill_sphere_shell(center_position)` (line 564) -- Creates the Moon's Hill sphere.

---

### neptune_visualization_shells.py

**Role:** rendering/shells | **Lines:** 1,557
...
- `create_neptune_magnetic_poles(center_position, offset_distance, tilt, azimuth)` (line 682) -- Creates a simplified visualization of Neptune's magnetic poles and axis.
- `create_neptune_radiation_belts(center_position)` (line 826) -- Creates Neptune's radiation belts with proper structure reflecting the complex magnetospheric environment.
- `create_field_aligned_currents(mag_center_x, mag_center_y, mag_center_z, tilt, azimuth)` (line 1051) -- Creates visualization of field-aligned currents in Neptune's magnetosphere.
- `create_neptune_ring_system(center_position)` (line 1193) -- Creates a visualization of Neptune's ring system with proper alignment.
- `create_neptune_hill_sphere_shell(center_position)` (line 1690) -- Creates neptune's Hill sphere shell.

---

### planet9_visualization_shells.py

**Role:** rendering/shells | **Lines:** 267
...
- `create_planet9_surface_shell(center_position)` (line 43) -- Creates eris's cloud layer shell.
- `create_planet9_hill_sphere_shell(center_position)` (line 219) -- Creates Planet 9's Hill sphere shell.

---

### pluto_visualization_shells.py

**Role:** rendering/shells | **Lines:** 565
...
- `create_pluto_core_shell(center_position)` (line 39) -- Creates pluto's core shell.
- `create_pluto_mantle_shell(center_position)` (line 129) -- Creates pluto's mantle shell.
- `create_pluto_crust_shell(center_position)` (line 214) -- Creates pluto's cloud layer shell.
- `create_pluto_haze_layer_shell(center_position)` (line 383) -- Creates pluto's haze layer shell.
- `create_pluto_atmosphere_shell(center_position)` (line 483) -- Creates pluto's atmosphere shell.
- `create_pluto_hill_sphere_shell(center_position)` (line 576) -- Creates pluto's Hill sphere shell.
...
**Role:** rendering/shells | **Lines:** 1,312
...
- `create_sun_gravitational_shell()` (line 937) -- Creates the Sun's gravitational influence shell.
- `create_sun_outer_oort_shell()` (line 961) -- Creates the Sun's outer Oort cloud shell.
- `create_sun_inner_oort_shell()` (line 987) -- Creates the Sun's inner Oort cloud shell.
- `create_sun_inner_oort_limit_shell()` (line 1013) -- Creates the inner limit of the Sun's Oort cloud shell.
- `create_sun_heliopause_shell()` (line 1039) -- Creates the Sun's heliopause shell.
- `create_sun_termination_shock_shell()` (line 1064) -- Creates the Sun's termination shock shell.
- `create_sun_outer_corona_shell()` (line 1090) -- Creates the Sun's extended outer corona (F-corona) shell.
- `create_sun_inner_corona_shell()` (line 1114) -- Creates the Sun's inner corona (K-corona) shell.
- `create_sun_streamer_belt_shell()` (line 1138) -- Visible white-light corona / helmet streamer belt: ~4-6 solar radii.
- `create_sun_roche_limit_shell()` (line 1168) -- Fluid Roche limit for cometary bodies: ~3.45 solar radii (~0.016 AU).
- `create_sun_alfven_surface_shell()` (line 1203) -- Alfven surface: the true outer boundary of the solar corona (~18.8 solar radii,
- `create_sun_chromosphere_shell()` (line 1233) -- Creates the Sun's chromosphere shell.
- `create_sun_photosphere_shell()` (line 1257) -- Creates the Sun's photosphere shell (the visible solar surface).
- `create_sun_radiative_shell()` (line 1282) -- Creates the Sun's radiative zone shell.
- `create_sun_core_shell()` (line 1306) -- Creates the Sun's core shell.
- `create_sun_hills_cloud_torus(center_position, inner_radius, outer_radius, thickness_ratio)` (line 1338) -- Create a toroidal (doughnut-shaped) Hills Cloud structure.
- `create_sun_outer_oort_clumpy(center_position, radius_min, radius_max, n_clumps)` (line 1402) -- Create a clumpy, asymmetric outer Oort Cloud with density variations.
- `create_sun_galactic_tide(center_position, radius, n_points)` (line 1478) -- Create Oort Cloud structure influenced by galactic tidal forces.

---

### uranus_visualization_shells.py

**Role:** rendering/shells | **Lines:** 1,081
...
- `create_uranus_radiation_belts(center_position)` (line 618) -- Creates Uranus's radiation belts.
- `create_uranus_ring_system(center_position)` (line 800) -- Creates a visualization of Uranus's ring system.
- `create_uranus_hill_sphere_shell(center_position)` (line 1153) -- Creates Uranus's Hill sphere shell.
...
- `create_venus_core_shell(center_position)` (line 44) -- Creates Venus's core shell.
- `create_venus_mantle_shell(center_position)` (line 113) -- Creates Venus's mantle shell.
- `create_venus_crust_shell(center_position)` (line 180) -- Creates Venus's crust shell using Mesh3d for better performance with improved hover.
...
- `add_keplerian_position_marker(fig, obj_name, orbital_params, current_datetime, rotate_points, center_body)` (line 1753) -- Add a Keplerian (analytical) current position marker to the plot.
- `compute_pairwise_encounter(sc_positions, target_positions, sc_dates, target_dates)` (line 1857) -- Find the closest approach between two objects from their position time series.
- `add_encounter_marker(fig, encounter, sc_name, target_name, color_map, obj_info)` (line 1978) -- Add an encounter marker to a 3D Plotly figure.
...
- `save_visualization(fig, default_name, mode, output_path, offline, auto_play, open_browser)` (line 579) -- Unified save function for all Plotly visualizations.
- `show_and_save(fig, default_name, auto_play)` (line 821) -- Show visualization in browser, then offer save dialog.
- `save_plot(fig, default_name)` (line 885) -- Legacy function for backward compatibility.
- `handle_save(fig, default_name)` (line 903) -- Legacy function for backward compatibility.
- `show_animation_safely(fig, default_name)` (line 916) -- Show and optionally save an animation.
- `save_html(fig, filename, offline, open_browser)` (line 937) -- Simple direct save for scripts that don't need dialogs.
...
- `calculate_planet9_position_on_orbit(a, e, i, omega, Omega, theta)` (line 217) -- Calculate position that lies exactly on the orbit defined by the parameters
- `rotate_points2(x, y, z, angle, axis)` (line 265) -- Rotates points (x,y,z) about the given axis by 'angle' radians.
- `calculate_axis_range(objects_to_plot)` (line 313) -- Calculate appropriate axis range based on outermost planet
- `fetch_trajectory(object_id, dates_list, center_id, id_type, start_date, end_date)` (line 330) -- Fetch trajectory data in batch for all dates, handling missing epochs through interpolation.
- `fetch_orbit_path(obj_info, start_date, end_date, interval, center_id, id_type)` (line 556) -- Fetch orbit path data from JPL Horizons for the given object between start_date and end_date,
- `pad_trajectory(global_dates, object_start_date, object_end_date, object_id, center_id, id_type)` (line 606) -- Fetch trajectory and pad with None before start_date and after end_date.
- `add_url_buttons(fig, objects_to_plot, selected_objects)` (line 641) -- Add URL buttons for missions and objects in solar system visualizations.
- `get_default_camera()` (line 751) -- Return the default orthographic camera settings for top-down view
- `print_planet_positions(positions)` (line 763) -- Print positions and distances for planets.
- `create_orbit_backup()` (line 793) -- Create a backup of orbit cache on startup
- `cleanup_old_orbits()` (line 826) -- Remove orbit data older than 30 days
- `show_animation_safely(fig, default_name)` (line 898) -- Show and optionally save an animation.
...
**Role:** utility | **Lines:** 202

> shared_utilities.py - Small shared helpers used across shell visualization modules.

**Depends on:** orrery_rendering
**Consumed by:** comet_visualization_shells, moon_visualization_shells, neptune_visualization_shells, palomas_orrery, planet_visualization, smoke_phase4, uranus_visualization_shells

**Public functions:**

- `traces_extent_from_center(traces, center)` (line 25) -- Max distance (AU) of any trace vertex from a center point, over a
- `create_sun_direction_indicator(center_position, sun_position, axis_range, shell_radius, object_type, center_object, body_name)` (line 57) -- Creates a visual indicator arrow pointing from the body toward the Sun.
...
- `extract_figure_and_frames(html)` (line 36) -- Return (data_list, frames_list, frames_json_bytes) from a saved
- `report(path)` (line 66)
...
- `create_info_marker(x, y, z, color, text, legendgroup, customdata, fill_color, border_color)` (line 37) -- Create a single info marker trace.
...
**Consumed by:** palomas_orrery, planet_visualization, smoke_dipole_cone, smoke_rotation_axis
...
> Phase 4 live-dispatch smoke test. Imports palomas_orrery (GUI builds under xvfb; mainloop stubbed), drives the REAL checkbox vars, and asserts: 1. Opt-in OFF: checked magnetosphere absent from collect specs and the center skip set; '(static plots only)' legend placeholder PRESENT. 2. Opt-in ON: m...
...
- `specs_for(center)` (line 34)
- `placeholder_present(center)` (line 37)

---

### smoke_rotation_axis.py

**Role:** other | **Lines:** 145

> smoke_rotation_axis.py -- container smoke test for the rotation-axis primitive (Movement 2, June 2026). Exercises the LIVE dispatch wiring: it resolves each body's CUSTOM_SHELLS['<body>']['rotation_axis']['builder'] string and calls it exactly as planet_visualization.py does (planet_name passed v...
...
- `resolve_builder(body)` (line 45) -- Resolve the builder the same way the dispatch loop does.
- `axis_line(traces)` (line 54)
- `all_arcs(traces)` (line 61)
- `main()` (line 69)
...
| apsidal_markers | computation | 1,738 | 1 | 4 |
...
| comet_visualization_shells | rendering/shells | 1,850 | 3 | 1 |
...
| neptune_visualization_shells | rendering/shells | 1,557 | 4 | 2 |
...
| orrery_rendering | other | 318 | 2 | 14 |
...
| visualization_utils | rendering | 854 | 3 | 3 |

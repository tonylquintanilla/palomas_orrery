...
**Apsidal Marker Validation & Perturbation Visualization**: The software provides visual proof of orbital perturbations by displaying both theoretical and actual apsidal points at the same dates. The ideal markers show where perihelion/aphelion would occur in a perfect Keplerian orbit at the time when the actual periapsis passage occurs. The spatial difference between ideal and actual markers directly visualizes the effect of gravitational perturbations from other bodies, demonstrating how real orbits deviate from simple two-body dynamics.
...
This guide provides the official, tested method for setting up Paloma's Orrery on Windows. Mac/Linux users may need to adapt some commands, particularly for GUI threading operations.
...
- ~2GB free disk space (for cache files and astronomical databases)
...
4.  Important: Note this location - you'll need to navigate here in the next step

### Step 3: Install Required Libraries

## Option A: Quick Installation (All at once)
Type this command and press Enter:
   pip install -r requirements.txt
This installs everything in one go. Takes 2-5 minutes.

## Option B: Step-by-Step Installation (Recommended for beginners)
Install each library group separately to understand what you're adding:
1. Core Mathematical Libraries (The foundation - install these first!)
Type this command and press Enter:
   pip install numpy pandas scipy

- numpy - Handles arrays and mathematical operations (like a super calculator)
- pandas - Manages data in table format (like Excel for Python)
- scipy - Scientific computing tools (advanced math functions)

2. Astronomy Libraries (The space stuff!)
Type this command and press Enter:
   pip install astropy astroquery erfa

- astropy - Core astronomy calculations (planet positions, time conversions)
- astroquery - Fetches data from NASA/ESA databases (gets real asteroid orbits!)
- erfa - Essential astronomy algorithms (Earth rotation, precession)

3. Visualization Libraries (Makes the pretty pictures)
Type this command and press Enter:
   pip install plotly kaleido pillow

- plotly - Creates interactive 3D visualizations you can rotate
- kaleido - Saves your plots as PNG/JPG images
- pillow - Image processing (required by kaleido)

4. GUI Libraries (The user interface)
Type this command and press Enter:
   pip install customtkinter

- customtkinter - Modern-looking buttons and controls
Note: Regular tkinter comes with Python - no install needed!

5. Web and Data Libraries (Internet connectivity)
Type this command and press Enter:
   pip install requests beautifulsoup4 html5lib python-dateutil pytz

- requests - Downloads data from the internet
- beautifulsoup4 - Reads web pages (for scraping astronomical data)
- html5lib - Helps beautifulsoup4 parse HTML
- python-dateutil - Advanced date/time handling
- pytz - Timezone support (important for astronomy!)

6. Optional but Recommended (Nice to have)
Type this command and press Enter:
   pip install h5py astropy-healpix pyvo

- h5py - Reads special astronomy data files
- astropy-healpix - Sky coordinate system used by astronomers
- pyvo - Access to Virtual Observatory services

7. Optional Development Tools (If you want to modify the code)
# Recommended: Visual Studio Code (VS Code)
- VS Code is a free, user-friendly code editor that makes reading and modifying Python code much easier than using Notepad or command-line tools.
- To install VS Code:
   ...
   3. Open VS Code and install the Python extension:
      ...
      2. Search for "Python"
      3. Install the one by Microsoft (usually first result)

- Open the palomas_orrery folder in VS Code:
   File → Open Folder → Navigate to your palomas_orrery folder

- VS Code benefits for this project:

   - Color-coded syntax - Makes code much easier to read
   - IntelliSense - Shows you available functions as you type
   - Integrated terminal - Run the program without leaving VS Code (View → Terminal)
   - Error highlighting - Red squiggly lines show problems before you run
   - Search across files - Ctrl+Shift+F to find anything in the project
   - Git integration - Track your changes if you modify the code

### Step 4: Run the Application

# Method 1: Direct Launch (Simplest)
   1. Navigate to the project folder in Windows Explorer
   2. Double-click palomas_orrery.py
...
# Method 3: VS Code (Recommended for developers)
   ...
   2. Open palomas_orrery.py and press F5 or click Run

# Important VS Code Setting:
Configure VS Code to execute Python files in their containing directory:
...
2. **Dual marker system for perturbation visualization**:
   - **Ideal apsidal points**: Show where perihelion/aphelion would occur in an unperturbed Keplerian orbit at the time of actual periapsis passage
   - **Actual apsidal markers**: Fetched from JPL Horizons showing true perturbed positions at the same dates
   - **Direct perturbation comparison**: The spatial separation between ideal and actual markers at the same date reveals orbital perturbations
   - **Consistent positioning**: All visualizations use actual JPL Horizons positions for accuracy

3. **Enhanced information display**:
   - Hover over markers to see predicted dates and distances
   - Ideal markers show "Unperturbed Keplerian position at actual periapsis time"
   - Actual measured distances displayed at true apsidal dates
   - Automatic terminology adjustment (perihelion/aphelion for solar orbits, perigee/apogee for moons)

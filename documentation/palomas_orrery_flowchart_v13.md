flowchart TD
 subgraph subGraph7["External Data Sources"]
        ...
        CLIMATESRC{{"Climate Data Sources"}}
        PALEOSRC{{"Paleoclimate Data Sources"}}
 end
 ...
 subgraph subGraphCache["Cache Management"]
        ...
        CLIMATECACHE(["climate_cache_manager.py"])
        BACKUPS(["create_cache_backups.py"])
        VERIFYORBIT(["verify_orbit_cache.py"])
        ...
        VALIDATECLIMATE[/"validate_climate_cache"\]
        REPAIRCACHE[/"repair_cache_on_load"\]
 end
 ...
 subgraph subGraph8["Data Processing"]
        ...
        CONVERTHOT(["convert_hot_ph_to_json.py"])
 end
 ...
 subgraph subGraphSolar["🌟 SOLAR SYSTEM PIPELINE"]
     ...
     subgraph subGraph10Solar["Solar Visualization Processing"]
            ...
            PLANVIZUTIL(["planet_visualization_utilities.py"])
     ...
     subgraph subGraph12Solar["Planetary Shell Modules"]
            ...
            ASTEROID_SHELLS(["asteroid_belt_visualization_shells.py"])
            COMET_SHELLS(["comet_visualization_shells.py"])
     end
 ...
 subgraph subGraphEarth["🌍 EARTH SYSTEM PIPELINE"]
     subgraph subGraphClimate["Climate Data Processing"]
            CLIMATEVIZ(["earth_system_visualization_gui.py"])
            PALEOFULL(["paleoclimate_visualization_full.py"])
            PALEODUAL(["paleoclimate_dual_scale.py"])
            CLIMATEUI[/"Climate Control Panel"\]
            VISUALIZEDATA[/"create_climate_visualizations"\]
            PALEOPLOTS[/"create_paleoclimate_plots"\]
     end
 end

 subgraph subGraphExoplanets["🪐 EXOPLANET PIPELINE"]
     subgraph subGraphExo["Exoplanet Systems"]
            ...
            EXOSYSDEF[/"exoplanet_system_definitions"\]
            EXOCALC[/"calculate_exoplanet_orbits"\]
            EXOCOORDTRANS[/"exoplanet_coordinate_transforms"\]
     end
 end
 ...
 subgraph subGraph5["Configuration & Educational Content"]
        ...
        EXODATA[/"exoplanet_data definitions"\]
     end
 ...
 subgraph subGraphOutputs["📝 FINAL OUTPUTS"]
        ...
        CLIMATECACHEDIR[("`climate_cache/`")]
        ORBITCACHEDIR[("`orbit_cache/`")]
 end
    ...
    %% Orbital Parameter Visualization Flows
    ORBVIZ ==> CREATEORBVIZ
    ORBVIZ ==> CREATEORBWIN
    CREATEORBVIZ ==> A
    CREATEORBWIN ==> A
    ...
    ASTEROID_SHELLS ==> PLOTOBJECTS
    COMET_SHELLS ==> PLOTOBJECTS
    ...
    %% EARTH SYSTEM PIPELINE FLOWS (DOTTED ARROWS)
    CLIMATESRC -..-> FETCHCLIMATE
    PALEOSRC -..-> FETCHPALEO
    FETCHCLIMATE -..-> CLIMATECACHE
    FETCHPALEO -..-> CLIMATECACHE
    CLIMATECACHE -..-> VALIDATECLIMATE
    VALIDATECLIMATE -..-> CLIMATEVIZ
    CLIMATEVIZ -..-> CLIMATEUI
    CLIMATEUI -..-> VISUALIZEDATA
    FETCHCLIMATE -..-> CONVERTHOT
    CONVERTHOT -..-> JSONFILES
    PALEOFULL -..-> PALEOPLOTS
    PALEODUAL -..-> PALEOPLOTS
    PALEOPLOTS -..-> PNGFILES
    VISUALIZEDATA -..-> PNGFILES
    VISUALIZEDATA -..-> CLIMATECACHEDIR
    A -..-> CLIMATEVIZ

    %% EXOPLANET PIPELINE FLOWS (DASH-DOT ARROWS)
    H -.-> EXODATA
    EXODATA -.-> EXOSYS
    EXOSYS -.-> EXOSYSDEF
    EXOSYSDEF -.-> EXOORB

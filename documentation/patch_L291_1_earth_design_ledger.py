"""
patch_L291_1_earth_design_ledger.py

Records the 2026-09-06 evening session in the ORRERY repo:
  - LEDGER_CONSOLIDATED.md
      L-288 updated: built, run, Mode 5 passed by the Studio path;
                     narrowed Gap (the json_converter path is untested).
      L-289 CLOSED:  Mode 5 results, patches 5 and 6, Tony's rulings.
      L-291 NEW:     Earth exhibit -- design settled, build pending.
      L-292 NEW:     Earth shells the orrery does not draw (class row).
      L-293 NEW:     Lunar standstill exhibit.
      L-294 NEW:     The Explorer room's placeholder and the heliocentric
                     Earth view.
      L-295 NEW:     Upper atmosphere shell disagrees with its own hover.
      L-296 NEW:     Every design build earns a plan version number.
  - documentation/MASTER_PLAN_INTERACTIVE_GALLERY.md
      v25 -> v26 (Tony's rule, L-296: a design build earns a version).
      Status block: the stale "phone pass is the one thing carried"
      sentence corrected; Earth named as designed-not-built.
      One appended "you are here" subsection.
...
<!-- L:296 status:OPEN upd:2026-09-06 section:A flag: rice:3/3/90/1 -->
...
- **VERSION COLLISION, RULED.** This ruling and L-290's relay-anchor
  amendment both belong in `ledger-and-session-records`, and both
  wanted 1.10. Tony, 2026-09-06: **"one session - one bump."** Both
  land in a single 1.10, in whichever session does the work; a session
  does not ship two versions of one skill. That makes L-290's pending
  ruling the GATE for this item too -- neither amendment ships until
  the relay-anchor drafts are accepted, amended or declined.
- **Note:** RICE 3/3/90/1 -> 8.1 proposed, not confirmed. High because
  ...
  update.
**Gap:** Tony's ruling on L-290's drafts; then ONE bump to 1.10
carrying both amendments, under the four-step binding rule (SKILL.md,
skills_index.py, a protocol version-history entry, one commit),
verified by the NEXT session's load per Stale Skill = Stop.
**Ref:** L-290 (the other pending 1.10), L-291 (the design build that
...
L290_ADD_NEW = """- **What v3.53 ALREADY covers, checked at orrery `50cbd2df`
  2026-09-06** so the amendment is scoped to the real gap: HOW each
  partner reads the repo is written under AI Roles from L-276 (Claude
  and GPT fetch live at a pinned SHA; Gemini's web app imports one
  public repo as a snapshot of HEAD, cannot read a URL given in a
  prompt, cannot fetch at a SHA, cannot see commit history, and is not
  on mobile). The "Documents as handoffs" clause already requires
  `built on <SHA> at <URL>` on every outbound document, with a reason
  covering both kinds of partner, and the Anchor Requirement gate
  applies it uniformly. THE GAP IS NARROWER THAN "anchors": it is that
  none of that names `PROJECT_INSTRUCTIONS.md` or the task-relevant
  `SKILL.md` files as fetch targets, so a partner gets the code with
  none of the governance and reads what was built without the rules the
  build ran under.
- **This item now carries L-296 too.** Tony's ruling 2026-09-06, "one
  session - one bump": both amendments land in a single
  ledger-and-session-records 1.10, so THIS item's decision gates both.
- **Tony-action (decide):** accept, amend or decline the two drafts.
**Gap:** Tony's ruling; then the protocol edit, ONE skill bump to 1.10
carrying both this and L-296, the reinstall, and the manifest
regeneration, in one push.
"""
...
def main():
    ...
    print("")
    print("Done. 8 edits across 2 files.")
    print("")
    ...
    print("  L-289 CLOSED")
    print("  L-290 updated (carries L-296 in one 1.10; gap narrowed)")
    ...
    print("  L-296 Design builds earn a plan version number     [NEW]")
    print("")
    ...
    print("  one appended 'you are here' subsection, 2026-09-06 (evening)")
    print("")
    print("NEXT: run ledger_index.py from this same directory. It")
    print("regenerates the index zone and migrates the DONE L-289 block to")
    print("its closed bucket. Then commit and push both repos.")
    print("")
    print("STILL OPEN, and it blocks the Earth build:")
    print("  install the interactive-exhibit skill (Settings > Skills).")

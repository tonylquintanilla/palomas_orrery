# HANDOFF: Shell Consolidation -- Consolidated Ledger + Go-Forward Plan (v21)

**Date:** May 31, 2026
**Session model:** Claude Opus 4.8
**Supersedes:** v20 -- single running ledger (carries the full ledger forward)
**Type:** D-URANUS render-and-diagnose session (Mode 5 + investigation). NO code
changed -- this was a guided render session that diagnosed root causes and
converged the fix design; implementation deferred to a single design/implement
session by Tony's call. Deltas vs v20: U2 CLOSED by convention (not render-
settleable -- principled reason recorded); U3 + belt/ring mismatch ROOT-CAUSED
(105 is an admitted fudge; the correct pole-vector transform already exists in
idealized_orbits.py and both belts+rings ignore it); U1 value decision still
Tony's pick; U4 unchanged; three feature candidates logged (pole markers,
dipole sweep-cone, bow shocks Uranus+Neptune); bow shocks confirmed as item 24 /
Mode 7 (not folded into the frame fix).
**Integrator:** Tony Quintanilla

> **This is the one document to follow.** v20 is superseded and retired. Its
> full ledger is carried forward intact (nothing renumbered). This session
> changed no code: it re-scoped the go-forward plan (v19), then investigated
> items 49 and 53 and read the Uranus file end-to-end (v20). The ledger rule
> still holds: nothing renumbered; new items appended (N10, N11 added this
> session).
...
| N2-orphan | Uranus magnetic tilt SIGN verification | uranus (60 deg) | **CLOSED by convention (v21).** Was expanded to D-URANUS U2 as "render-gated"; the render session found it is NOT render-settleable (no axial-rotation model -> dipole azimuth undefined). +60 retained as a documented display convention; reopen if axial rotation is ever modeled. |
...
- **U3 (105 fudge) + belt/ring mismatch -- ROOT-CAUSED. The fix is simpler than
  ...
  - **THE ONE OPEN QUESTION** that sets clean-swap vs needs-a-pre-step: what
    ...
    something else -> needs a frame-matching pre-step. **Answered by reading the
    belt/ring GENERATION blocks (pre-rotation point construction)** -- this grep
    is deliberately deferred to the HEAD of the implement session (against
    files confirmed current then, not read now and gone stale). It is the first
    action of that session and the design-then-decide trigger.

- **U1 (59 vs 60) -- still Tony's value pick** (unchanged from v20; 1 deg,
  invisible in render). Recommend L503 -> 59, soften L519 to "nearly 60." Do
  this in the same implement session as U3 (same load-bearing token at L503).

- **U4 (Saturn copy-paste comments) -- unchanged**, batch with the implement
  session (see v20 line map below for the 5 error sites vs the legitimate Saturn
  comparisons to leave alone).
...
2. **D-URANUS -- DIAGNOSED (v21); one design/implement session remains.** The
   render session (Tony, May 31) resolved the diagnosis -- see the **v21
   RENDER-SESSION RESULTS** block in the D-URANUS section, which supersedes the
   v20 per-item status. Standing now:
   - **U2 -- CLOSED by convention** (not render-settleable; no axial-rotation
     model -> dipole azimuth undefined; retain +60, document why). One comment
     line, rides with the implement session.
   - **U3 + belt/ring mismatch -- root-caused; fix design converged.** 105 is an
     admitted fudge; the correct pole-vector transform already exists in
     idealized_orbits.py (the moon-orbit path). Fix = extract
     `orient_to_planet_pole()` and route belts + rings through it; retire 105.
   - **U1 -- Tony's value pick** (recommend 59); **U4 -- comment cleanup.**
   - **First action of the implement session:** grep the belt/ring GENERATION
     blocks (the one open frame question -- clean swap vs pre-step). Deferred to
     session head deliberately (files current then).
   - **Preconditions:** confirm CURRENT uploads of `uranus_visualization_shells.py`
     AND `idealized_orbits.py` (producer lives there) + `planet_visualization_
     utilities.py` + `orrery_rendering.py`; verify-against-handoff each.
   - Data-content + geometry change -> xvfb smoke test on the LIVE dispatch, then
     Tony Mode-5 re-render (belts + moon orbits: do they now share the plane?).

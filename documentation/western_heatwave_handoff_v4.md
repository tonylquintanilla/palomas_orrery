# Paloma's Orrery -- Western Heatwave Timeline Handoff v4
...
**Fix:** Improve the HTML stripping to preserve line breaks with
double spacing for visual separation. In engine `create_intel_card()`:
...
# AFTER:
    # Convert <br> to double newlines, then strip remaining HTML tags
...
Each `<br>` becomes a double line break in the matplotlib text.
Bold/italic tags are stripped but visual grouping survives.

KMZ-only change -- regenerate KMZs, copy to `gallery/assets/`, push.

-----

## Gen-Mobile Studio Preset (Designed, Partially Implemented)

A new "Gen - Mobile" preset button for Gallery Studio, optimized for
mobile/portrait gallery display of earth system teasers.

### What's Decided

**Preset config (`GEN_MOBILE_CONFIG`):**
- Green background (#2d6a2d)
- No title (gallery viewer provides its own title bar)
- Show annotations: True (with shorter mobile briefing -- see below)
- No legend
- Tight margins (20 all around)
- Colorbar on, modebar on
- Output format: landscape

**KMZ link preservation:** The apply method saves and restores the
current KMZ link so the preset doesn't blank it:
...
**Three edits needed in `gallery_studio.py`:**
1. Add `GEN_MOBILE_CONFIG` dict after `PORTRAIT_CONFIG` (~line 204)
2. Add "Gen - Mobile" button in preset row (~line 3827)
3. Add `_apply_gen_mobile_preset()` method (~line 4758)

### What's Open: Mobile Briefing

The full briefing annotation is too long for mobile landscape view.
Tony wants a shorter `mobile_briefing` (title + detail + data
provenance only, no attribution/CSI/pins note):
...
mobile_briefing = (
    ...
    f'{detail}<br>'
    f'<br>'
    f'<i>Data: Preliminary synthetic. ERA5 reanalysis ~April 2026.</i>'
)
```

**The problem:** The briefing annotation is baked into the teaser HTML
at generation time by `generate_plotly_teaser()`. Studio can show/hide
annotations but can't swap their text. Studio loads a standalone HTML
file and doesn't have access to the scenario dict's `mobile_briefing`.

**Proposed solution (Option A -- embed both):** The engine embeds both
`briefing` and `mobile_briefing` in the layout dict (e.g., as
`_mobile_briefing`, similar to how `_kmz_handoff` and `_studio_config`
are already embedded). Studio gains a "Use mobile briefing" checkbox
in the Annotations section. When checked, it swaps the annotation text
from `_mobile_briefing` before export.

This follows the existing pattern -- the engine already embeds
metadata in the layout for Studio to consume downstream.

**Implementation steps (next session):**
1. Add `_build_mobile_briefing()` to scenario file
2. Add `mobile_briefing` key to scenario dicts via `_make_scenario()`
3. Engine embeds `_mobile_briefing` in layout alongside the annotation
4. Studio adds "Use mobile briefing" checkbox in Annotations section
5. Checkbox reads `_mobile_briefing` from layout and swaps annotation
   text when checked
6. Gen-Mobile preset enables `show_annotations: True` + checks the
   mobile briefing checkbox

**Alternative approaches considered:**
- Option B: Generate two separate teasers (doubles files)
- Option C: Editable annotation text field in Studio (more flexible
  but more UI complexity)

### What's Open: Pin Info Card Duplication

In portrait mode, tapping a map point triggers both Plotly hovertext
AND the info card (the `plotly_click` handler in `index.html` line
1911 fires in portrait mode and parses trace text into a card).

Tony doesn't want both -- the hovertext is sufficient for the pin
data values. The info card is redundant and potentially confusing.

**Options to resolve:**
- Suppress info card for mapbox/scattermapbox traces specifically
- Add a layout flag (`_suppress_info_card`) that the gallery viewer
  checks before wiring the click handler
- Studio checkbox to control whether the export triggers info cards
...
| `western_heatwave_handoff_v2.md` | Superseded | Replaced by v3 |
| `western_heatwave_handoff_v3.md` | Superseded | Replaced by this v4 |
...
2. **Intel card line breaks** (Issue 7) -- replace HTML stripping
   logic in engine `create_intel_card()`, use `\n\n` for double
...
- **Gen-Mobile preset implementation** -- three edits to
  `gallery_studio.py` (config dict, button, apply method). Code
  is designed and ready to implement.
- **Mobile briefing pipeline** -- engine embeds `_mobile_briefing`
  in layout; Studio checkbox swaps annotation text. Design decided
  (Option A), implementation next session.
- **Pin info card duplication** -- resolve whether to suppress info
  card for mapbox traces in portrait mode. Decision next session.
...
- **Presets should preserve scenario-specific settings.** The
  Gen-Mobile preset must save/restore the KMZ link rather than
  blanking it. Presets set general layout; scenario-specific data
  (KMZ links, trace visibility) should survive preset application.
- **Mobile and desktop need different briefing lengths.** The full
  briefing works on desktop but overwhelms mobile. The engine should
  embed both versions; the Studio selects which to use at export time.
  This follows the existing pattern of embedding metadata in the layout

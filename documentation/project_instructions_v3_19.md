...
## Tony with Claude | v3.19 | April 13, 2026
...
- Single info marker pattern: hoverinfo='skip' on geometry + one cross marker at uncluttered position. North pole (r*1.05) for shells; named index for line traces (e.g. segment 10 on outbound arc). Separates visual density from interactivity. ~17 MB file size reduction. Eliminates routing log spam. April 2026.
- Shell n_points: outer/boundary=20, inner sun=25. Size=3.0-3.5, opacity=0.35-0.5 for visibility at 400 points. Photosphere/radiative/core unchanged (solid bodies).
- Ghost tail legendgroup: all segment traces must share legendgroup='maps_ghost_tail' (or equivalent). Without it, only the first segment (showlegend=True) responds to legend clicks; the rest stay visible permanently.
- Roche limit is not absolute: marks where tidal forces overcome self-gravity only. Tensile strength allows survival inside it. Ikeya-Seki (~5 km) survived at 1.66 R_sun; Great Comet of 1843 survived at 1.19 R_sun. Size and structural coherence are decisive.
- AU alongside solar radii: every solar radii citation must include the AU equivalent. Standing convention across all shell hover text and comet physics strings.
- Disintegration geometry verification: always compute with Barker's equation before writing hover text. MAPS disintegrated at 8.33 R_sun (0.039 AU) -- outside all inner shells, not inside as initially assumed.
- Credit line convention: add "# Module updated: [date] with Anthropic's Claude Sonnet 4.6" to module docstrings and design pattern comments when significant new work is added. Tony's standing convention: "Give credit where credit is due."
- Marker symbol convention: filled circle = major body, open circle = minor body, filled diamond = comet, open diamond = spacecraft, open square = structural position (Lagrange), cross (+) = non-structural hover target (ticks, info markers). Circles reserved for celestial objects. April 2026.
- Renderer refactor pattern: when rendering code is duplicated across plot_objects and animate_objects, extract into a single function in the source module. star_sphere_builder.py owns build + load + render. One import, one function call, zero parallel-pipeline divergence.
- Reuse existing markers for hover: when a tick/marker already exists at a position, add hovertext via customdata + hovertemplate rather than creating a duplicate marker trace. Fewer traces, cleaner rendering.
- Two-tier label system: persistent text at quarter positions (0/90/180/270) + dense hover on all ticks when Labels checkbox is on. Resolves tension between readability and clutter.
- Celestial sphere in ecliptic frame: stars and grid are unit vectors rotated from equatorial to ecliptic via obliquity rotation about X axis. All |r| = 1.0, scaled by axis_range at runtime. Prime meridian is RA=0h/12h great circle in equatorial XZ plane, rotated to ecliptic.
...
- Activation vs provision: For domains the LLM knows deeply, prompt engineering is *activation* not *provision*. Don't tell Napoleon about Waterloo -- tell him he's Napoleon and let what he already knows engage. Contrast with cooperative Mode 7 where the AI genuinely lacks domain knowledge. Two problems that look similar on the surface.
- The interpretation gap is signal: In any human-AI collaboration, the gap between AI output and human execution is not noise to minimize -- it's data about the quality of the AI's situational understanding. Measure it, log it, don't engineer it away. Impossible orders reveal where the AI's model of reality diverged from reality.
- Preserve the reasoning trace: When the AI's output is reasoning rather than calculation, the trace is more valuable than any single decision it produces. The move list is unreadable. The dispatch is history. In any domain where you're using an LLM to think rather than do, preserving the reasoning record is as important as implementing the output.
- Language over coordinates: LLMs reason better from prose description of spatial or structural relationships than from raw data. "The left flank is exposed with no reserve" beats a coordinate list. Applies wherever the AI needs to reason about position, structure, or relationship.
- Commitment order is semantics: When a sequence of decisions carries meaning (who advances, who absorbs losses, who is protected), encode that meaning in the order itself rather than in separate state variables. Position is state. Simplifies both the data structure and the reasoning about it.
- Two projects, one protocol: The protocol learns from both strands simultaneously. Orbital mechanics and Napoleonic warfare are different domains, but the structure that emerges -- the error-correction loop, the interpretation gap as signal, the reasoning trace as output -- is the same.
- Renderer refactor to reduce bloat: when inline rendering code reaches ~180 lines duplicated across two pipelines, extract into the source module as a callable function. The orrery shrinks, the source module grows its responsibility, and the parallel-pipeline anti-pattern is eliminated. Tony caught this one: "can we import some of this new code instead of adding more bloat."
...
- The fog of war is the experiment, not the obstacle. The error-correction loop only carries information when there is genuine uncertainty on both sides. Perfect execution under perfect information produces no signal worth having. The partnership is proven in the friction, not in the smooth parts.
- Imperfect translation between AI intent and human execution is structurally identical to real command friction. Generals have this problem too -- orders travel by courier, arrive garbled, get interpreted by subordinates with incomplete information. The last mile is not a failure of the system. It is the system working as complex collaboration actually works.
- The reasoning trace is the primary output. The session where the insight emerged is more valuable than the parameter it produced. The dispatch log is more valuable than the move list. The conversation is where the engineering happens -- document it accordingly.
- Two projects, one protocol. The double-helix operates at the meta level: orbital mechanics and Napoleonic warfare are different domains, but the structure that emerges from their interleaving -- the error-correction loop, the interpretation gap as signal, the reasoning trace as primary output -- is the same. The protocol learns from both strands simultaneously.
...
*~843 lines. Functional for Claude, readable for human, signal preserved.*

# Paloma's Orrery -- Western Heatwave Timeline Handoff v2
...
| `western_heatwave_handoff_v1.md` | Superseded | Replaced by this v2 |
...
- **True 3D extruded columns (Option B)** -- replace `newpoint()` with
  ...
  not urgent.
- **Intel card formatting** -- the KMZ intel card is a matplotlib PNG
  that only renders plain text (run-on after HTML stripping). Three
  options explored: (A) simulate formatting with multiple plt.text()
  calls, (B) render HTML to PNG via imgkit/weasyprint, (C) replace
  PNG with KML BalloonStyle HTML in a clickable placemark. Option C
  is cleanest -- GE natively renders HTML in description balloons,
  the briefing HTML already exists, no PNG generation needed. Tradeoff:
  click-to-see vs always-visible. Not critical. 

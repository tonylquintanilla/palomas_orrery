# HANDOFF: Shell Consolidation -- Consolidated Ledger + Go-Forward Plan (v23)

**Date:** June 1, 2026 (second session of the day)
**Session model:** Claude Opus 4.8
**Supersedes:** v22 -- single running ledger (carries the full ledger forward)
**Type:** D-URANUS U3 IMPLEMENT session (the deferred belt/ring geometry fix).
CODE CHANGED. Two files edited (`uranus_visualization_shells.py`,
`idealized_orbits.py`); both compile, ASCII-clean, LF; Tony Mode-5 render
CONFIRMS alignment. Deltas vs v22:
- **U3 CLOSED pending Tony sign-off** -- the 105 belt/ring fudge is RETIRED.
  Authored `orient_to_planet_pole()` in idealized_orbits.py (math core revived
  from the dead-tagged `create_planet_transformation_matrix`); routed belts +
  rings through it. Mode-5 render: rings, belts, and the classical moons now
  share one plane. Confirmed by Tony's eyes, not just the smoke test.
- **OBLIQUITY-FRAME BUG found and fixed mid-session** -- the first U3 build
  left belts/rings ~23.4 deg off the moons. Root cause: the IAU pole RA/Dec
  are EQUATORIAL (J2000); the plot is ECLIPTIC. The transform used the pole
  vector directly without the obliquity rotation. The container smoke test gave
  a FALSE PASS (it compared belts-to-rings and belts-to-the-same-wrong-pole,
  never to the moons -- it cannot fetch live Horizons data). Tony's render
  caught it. One added obliquity rotation fixed it; re-tested against the moon
  plane (independent ground truth) -> 0.0 deg.
- **MIRANDA tilt logged as CONFIRMED-CORRECT (new item N14)** -- Tony's eye
  noticed Miranda's orbit leans off the other classical moons. It is REAL
  (i = 4.232-4.34 deg to Uranus's equator, ~10x the other major moons; sourced),
  present in the JPL data, NOT a transform artifact. N14 captures it as a
  tooltip/inclination-anomaly note for celestial_objects.py or info_dictionary.py.
- **Shared-transform behavior-change note** -- the obliquity fix lives in the
  shared `create_planet_transformation_matrix`, so future callers
  (orient_to_planet_pole, bow shocks, other planets) inherit the correct frame.
  Today only the Uranus belts/rings consume it, so nothing else changed.

---

## v22 HEADER (retained for the record)
...
## THIS SESSION (June 1, second) -- D-URANUS U3 IMPLEMENT -- CODE CHANGED

Executed the deferred belt/ring geometry fix. Two files edited; Tony Mode-5
render confirms rings, belts, and the classical moons share one plane.

**The fix, and the design refinement made at session head.** v22 specified
"AUTHOR a new derived `orient_to_planet_pole`." On reading the uploads, the
dead-tagged `create_planet_transformation_matrix(planet_name)` was found to
already contain the correct, general, unfudged equatorial->ecliptic pole-vector
basis construction (node from pole, orthonormal basis). Decision (Tony OK'd
revive): use it as the math core, drop its DEAD tag, and add a thin
`orient_to_planet_pole(x, y, z, planet_name)` wrapper that applies the matrix to
a point cloud. One source of truth for the pole math; generalizes to Neptune /
bow shocks. Trust basis UNCHANGED -- never rendered, so render validation still
gates it; reviving dead-but-correct math avoids a duplicate, it does not inherit
trust. NOT the Neptune dipole (Tony asked): the Neptune/Triton path uses
sequential Euler rotations + its own hand-tuned "3 deg fine-tuning" step --
different, separately-fudged code. The matrix builder was an early general
utility, written correctly, never wired.

**The 11 U3 edits.** idealized_orbits.py: revived the matrix fn, added
`orient_to_planet_pole`, fixed an RA prose typo (257.31 -> 257.43, reconciled to
the authoritative `planet_poles['Uranus']`). uranus_visualization_shells.py:
imported the producer, deleted both `uranus_tilt = np.radians(105)` constants,
routed the belt's single-X and the ring's compound-X+Y each through
`orient_to_planet_pole(..., 'Uranus')`, rewrote the ring docstring, added a
credit line. Transactional binary-mode patch, all anchors hit once.

**THE OBLIQUITY BUG (the real lesson of this session).** First render: belts
and rings co-planar with EACH OTHER but ~23 deg off the MOONS. Diagnosis,
proved in the sandbox against independent ground truth: the IAU pole RA/Dec are
EQUATORIAL (J2000) coordinates; the visualization is ECLIPTIC (J2000). The
matrix built the basis from the raw equatorial pole vector WITHOUT rotating it
into the ecliptic by the mean obliquity (23.439 deg). So the belt/ring plane sat
in the equatorial frame, ~23.4 deg off the ecliptic-native moon orbits. (The teal
"celestial equator, tilted 23.4 deg" circle in the legend IS that obliquity.)
Fix: one obliquity rotation about X applied to the pole vector before building
the node/basis, inside `create_planet_transformation_matrix`. Re-tested against
the MOON plane (obliquity-rotated pole, derived from first principles, NOT from
the same matrix) -> belt 0.0 deg, ring 0.0 deg, plane inclination 82.19 deg
(complement of the moons' 97.77 deg -- same plane). Tony's render confirmed.

**Why the smoke test lied (protocol lesson, reinforces v3.24).** The first
smoke test reported 0.000 deg and "pass." It checked belt==ring (both wrong the
same way) and belt==pole-vector (the SAME wrong pole the matrix used) -- a
circular reference with no independent ground truth. It never compared against
the moons, because the container cannot fetch live Horizons data. "A smoke test
of the wrong path passes falsely; handoffs are claims, the render is fact" --
exactly. Tony's Mode-5 render was the ground truth and overruled the green
check. The corrected sandbox test now uses an INDEPENDENT ground truth (the
obliquity-rotated pole derived from first principles + the moons' reported
inclination), not the transform's own output.

**MIRANDA (Tony's eye, confirmed real -- new item N14).** Tony noticed Miranda's
orbit tilts off the other classical moons. Checked the run's own JPL osculating
inclinations: Ariel 97.72, Umbriel 97.71, Titania 97.76, Oberon 97.91 (tight
cluster) vs **Miranda 98.56** (outlier). Sourced: Miranda's inclination to
Uranus's equator is 4.232 deg (Wikipedia/Voyager) to 4.34 deg (some NASA-derived
listings) -- the highest of the major moons, ~10x the others, attributed to a
past 3:1 resonance with Umbriel that also drove the tidal heating behind its
"Frankenstein" surface. The tilt is in the SOURCE DATA, not the transform (which
moves all geometry together and cannot single one moon out). CONFIRMED-CORRECT,
no fix. Logged as N14: note the inclination anomaly in a Miranda tooltip
(celestial_objects.py or info_dictionary.py) -- a scientific-storytelling hook,
Tony's call, not U3 scope.

**Shared-transform behavior change (flag for the record).** The obliquity
rotation now lives in the shared `create_planet_transformation_matrix`. Any
future caller via `orient_to_planet_pole` (bow shocks / item 24, other planets)
inherits the correct ecliptic frame automatically -- good. But the function's
behavior changed for any future caller; today only the Uranus belts/rings
consume it, so nothing else is affected. Noted so it is not a surprise later.

Verification: py_compile clean on both; ASCII; LF. xvfb import (no cycle:
uranus_visualization_shells -> idealized_orbits is one-way). Live-dispatch smoke
on the actual CUSTOM_SHELLS builders. Tony Mode-5 render: PASS (rings/belts/moons
co-planar; Miranda's lean is real physics).

**Residue (flagged, not in U3 scope):**
- `rotate_points` import in uranus_visualization_shells.py is now DEAD (U3
  removed its last uses). Harmless; one-line trim whenever. Deadness created by
  this session -- recorded rather than silently trimmed.
- A THIRD 105 survives at idealized_orbits.py L3699, in `plot_satellite_orbit`'s
  Uranus branch (the mean-element fallback; Uranus moons run the osculating path,
  so it rarely fires). Left untouched per scope. Candidate to route through
  `orient_to_planet_pole` later, or fold into the dead-code sweep once its
  liveness for Uranus moons is settled.
...
| N14 | Miranda inclination-anomaly tooltip | **NEW (v23).** Note Miranda's orbital tilt in celestial_objects.py or info_dictionary.py. Miranda's inclination to Uranus's equator is 4.232 deg (Wikipedia/Voyager) to 4.34 deg (some NASA listings) -- highest of the major moons, ~10x the others; attributed to a past 3:1 resonance with Umbriel (also the tidal-heating source behind its "Frankenstein" surface). CONFIRMED-CORRECT in the render (Tony's eye, v23) -- the lean is real JPL physics (osculating i=98.56 ecliptic vs the ~97.7 classical-moon cluster), not a transform artifact. A scientific-storytelling hook; Tony's call. Small editorial item. |
...
2. **D-URANUS -- COMPLETE (v23, June 1).** U1 (->60), U2 (comment), U4 (5
   fixes), + 7 dead-code tags (v22); U3 belt/ring geometry fix (v23). Standing:
   - **U1/U2/U4 -- CLOSED** (v22). See THIS SESSION (June 1).
   - **U3 (belt/ring frame) -- CLOSED pending Tony sign-off (v23).** 105 fudge
     RETIRED. `orient_to_planet_pole()` authored in idealized_orbits.py (math
     core revived from the dead-tagged `create_planet_transformation_matrix`);
     belts + rings routed through it. An obliquity-frame bug in the first build
     (~23 deg off the moons; equatorial pole used in an ecliptic plot) was found
     by Tony's render and fixed with one obliquity rotation; re-tested against
     the moon plane -> 0.0 deg. Mode-5 PASS: rings/belts/classical moons
     co-planar.
   - **Ness tilt UNCERTAINTY -- CLOSED (Tony, June 1):** decided 60 is enough;
     not sourcing the error bar. The U1 sig-figs call stands. Provenance scan
     clean.
   - **Item 24 (bow shocks)** is now unblocked as the first consumer of the
     validated `orient_to_planet_pole`. Opens with the standoff-distance/shape
     design pass (held separately, NOT folded into the frame fix). Next session.
...
```
D-URANUS U3 implement + ledger v23: Anthropic's Claude Opus 4.8 (June 1, 2026, 2nd session)
  -- CODE CHANGED. Retired the Uranus 105-deg belt/ring fudge. Revived the
     dead-tagged create_planet_transformation_matrix as the math core (unfudged
     general pole-vector basis; NOT the Neptune dipole, which is separately
     fudged); added orient_to_planet_pole() wrapper; routed belts + rings through
     it; reconciled an RA typo. First render showed ~23 deg misalignment to the
     moons: the IAU pole RA/Dec are EQUATORIAL but the plot is ECLIPTIC, and the
     transform omitted the obliquity rotation. The container smoke test PASSED
     FALSELY (circular -- compared the geometry to the same wrong pole, never to
     the moons it cannot fetch). Tony's Mode-5 render caught it. One obliquity
     rotation fixed it; re-tested against the moon plane (independent ground
     truth) -> 0.0 deg. Logged Miranda's tilt as N14 (confirmed-correct real
     physics, tooltip candidate). Flagged the shared-transform behavior change
     and two residues (dead rotate_points import, a third 105 in the satellite
     fallback path).
Integrator (v23): Tony Quintanilla -- ran the Mode-5 renders that caught the
  obliquity-frame bug the smoke test missed (the protocol's "trust your eyes"
  vindicated again); asked the provenance question that distinguished the revived
  math from the Neptune dipole; caught Miranda's lean and asked whether it was
  real; closed the Ness-uncertainty thread at 60; directed the N14 tooltip note.
...
*"The smoke test compared the geometry to itself and called it aligned. The render compared it to the moons and called it 23 degrees off. The render wins." -- June 2026*

*"Equatorial pole numbers in an ecliptic plot: the missing 23.4 degrees was the obliquity all along." -- June 2026*

*"Tony's eye found the one tilt that was supposed to be there." -- Miranda, June 2026*

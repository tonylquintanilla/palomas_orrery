# Paloma's Orrery -- Western Heatwave Timeline Handoff v7

## Session Handoff | March 22-25, 2026 | Claude Opus 4.6
...
(March 14-25+, 2026). World Weather Attribution classified it as
...
The event has a two-pulse structure: Pulse 1 (March 14-22) crested
with 14 states breaking all-time March records simultaneously.
Pulse 2 (March 24-27) is confirmed: Denver broke its own all-time
March record again on March 25 (87F, breaking 86F set March 21).
Over 200 additional daily records forecast. NWS: 42.5M people
under CSI 3+ conditions March 24-27.

The scenario module tells the story as a 7-snapshot timeline:

| Date     | Label          | CSI | Stations | Data Source |
|----------|----------------|-----|----------|-------------|
| March 14 | The Spark      | 2   | 3        | ERA5        |
| March 17 | The Pivot      | 3   | 6        | ERA5        |
| March 18 | Records Fall   | 4   | 9        | ERA5        |
| March 20 | The Peak       | 5   | 12       | ERA5        |
| March 21 | Eastward Surge | 5   | 23       | ERA5        |
| March 22 | The Crest      | 5   | 27       | Synthetic   |
| March 25 | Round Two      | 5   | 31       | Synthetic   |
...
March 22 ERA5 expected ~March 27. March 25 ERA5 expected ~March 30.
Re-running `generate_all()` will auto-upgrade. No code changes needed.
...
| File | Status | Notes |
|------|--------|-------|
| `scenarios_western_heatwave_march_2026.py` | Delivered (v3) | ERA5 CDS API, 7 snapshots, 31 stations, expanded grid |
...
| `western_heatwave_handoff_v1-v6.md` | Superseded | Replaced by this v7 |

-----

## On the Horizon

- **ERA5 auto-upgrade for March 22 and 25** -- re-running
  `generate_all()` after ~March 27 (for Mar 22) and ~March 30
  (for Mar 25) will automatically fetch ERA5. March 14-21 load
  from CSV cache instantly. No code changes needed.
- **Encyclopedia formal attribution** -- enrich the shared Data &
  Sources block in `_build_encyclopedia()` with formal Copernicus
  attribution language ("modified Copernicus Climate Change Service
  information [2026]"), NWS/NCEI citation, WWA reference, and
  Hersbach et al. (2020) citation. The "modified" qualifier is a
  license requirement because the pipeline computes anomalies from
  the raw ERA5 data. Gemini (Mode 7) provided the attribution
  language. Implementation: one text block edit, applies to all
  snapshots automatically via the shared encyclopedia builder.
- **March 25 station data confirmation** -- current March 25 values
  are based on NWS forecasts (Denver 87F confirmed, Las Vegas 98F
  and Lubbock 100F were forecasts). Update with actuals when NWS
  Record Event Reports are issued.
- **Event continuation assessment** -- cold front arriving March 26
  (Thursday). Determine whether the event ends or continues. If
  it ends, the 7-snapshot arc is complete. If it continues, add
  March 26+ snapshots.
- **Snowpack story** -- Colorado snowpack at 40% of median (historic
  low), Denver Stage 1 drought restrictions declared. Potentially
  a separate visualization (freshwater/cryosphere boundary) rather
  than air temperature anomaly.
- **Encyclopedia for existing scenarios** -- the `_build_encyclopedia`
  ...
  planetary boundary connections.
- **Unified KMZ timeline** -- single KMZ with dated folders. Would
  wrap all 7 snapshots in a single file with toggleable folders in
  GE sidebar.
- **CDS API key rotation** -- Tony's API key was briefly exposed in
  chat. Recommend regenerating on CDS profile page.
- **March 21 old ERA5 cache cleanup** -- when expanding the grid,
  old cached files (CSV, raw NC, climatology NC) must be deleted
  so the CDS API re-fetches with the new extent. Document this
  pattern for future grid expansions.

# Paloma's Orrery -- Biosphere Expansion Handoff v5

## Session Handoff | March 21, 2026 | Claude Opus 4.6

*Updated from v4 to capture completed Round 1 visual polish,
Gallery Studio Generator preset, unique legend filenames, and
full pipeline verification through gallery.*
...
### Step 3: Visual Polish Round 1 (Complete, Tested, Deployed)

Restored pre-KMZ-refactor visual style for all three GE legend cards
while retaining the config-driven engine architecture. The KMZ refactor
had inadvertently replaced the curated card designs with simplified
placeholders.

**Risk legend (`create_legend_card`):**
- Restored white semi-transparent background (alpha 0.7)
- Matplotlib `legend()` patches built from config bands
- `legend_title` from thresholds config (boundary-type-aware)
- Was: black text on transparent background (invisible over terrain)

**Intel card (`create_intel_card`):**
- Restored light `#f8f9fa` background (alpha 0.7)
- Compact `4 x 3.0` figure, fontsize 8, width=40 wrap
- Was: dark `#111111` military style, `4.5 x 5` figure with dead space

**Population legend (`create_pop_legend_card`):**
- Restored three-tier circles: red (Megacity >5M), orange (Major >1M),
  yellow (Region >500k) matching actual KML rendering
- Was: single blue circle (wrong color, wrong shape)

**Impact KML circles (`build_impact_kml`):**
- Restored tiered red/orange/yellow/white by population bracket
- Restored `math.sqrt(pop) / 75` sizing (was `pop / pop_divisor`)
- Names now include population count: `"New York City (7,800,000)"`

**Unique legend filenames (GE cache fix):**
- `legend_risk_index_cited.png` -> `legend_risk_{scenario_id}.png`
- `legend_impact_pop.png` -> `legend_pop_{scenario_id}.png`
- Eliminates GE temp file cache collisions between scenarios

**Gallery Studio Generator preset:**
- New `GENERATOR_CONFIG` dict and "Generator" button in preset row
- Green background `#2d6a2d`, no legend, annotations with transparent
  bg, modebar visible, colorbar visible
- Preserves per-scenario KMZ link and custom title on application

**Pipeline verification:** Full end-to-end tested: Generator ->
Studio (Generator preset) -> json_converter -> gallery viewer with
3D Earth button. All outputs verified.
...
### Round 1: Legend Cards + Intel Card (COMPLETE)

Restored pre-KMZ visual style for all three card types and the
impact KML circles. See "Step 3" above for details. Round 2 (intel
card) was absorbed into Round 1 since restoring the pre-KMZ style
addressed both the legend cards AND the intel card in one pass.

### ~~Round 2: Intel Card~~ (Absorbed into Round 1)
...
|------|-------|------|--------|
| `earth_system_generator.py` | 678 | Engine | Deployed, Round 1 complete |
...
| `gallery_studio.py` | ~5140 | Generator preset added | Deployed |
...
### Current: Visual Polish (Step 3 -- Rounds 3-4 remaining)

Round 1 complete (legend cards, intel card, impact circles, unique
filenames, Generator preset). Full pipeline verified.

Next: Round 3 (coral teaser stride) and Round 4 (legend-per-viz
routing). Both are targeted changes to `earth_system_generator.py`.
...
- **Restoring pre-refactor visuals:** The KMZ refactor inadvertently
  replaced curated card designs with simplified placeholders. Keeping
  the archived pre-refactor version enabled direct comparison and
  restoration. Lesson: archive before refactoring visual code.

- **GE temp file cache collisions:** Google Earth caches extracted KMZ
  contents by filename. Shared legend filenames (`legend_risk_index_
  cited.png`) cause stale overlays from previous scenarios to persist
  even when a new KMZ is opened. Fix: per-scenario filenames.

- **Studio presets must stash per-scenario values:** Presets wipe all
  config fields. Any per-scenario value (KMZ link, custom title) must
  be stashed before preset application and restored after. This
  pattern now applies to Generator, Portrait, and Landscape presets.

- **Work on the right file:** When project files include both pre-
  and post-refactor versions, verify which version is current before
  editing. Editing the wrong file wastes a full round of changes.
...
| Visual style approach? | Restore pre-KMZ designs | Curated visuals lost in refactor; archive had them |
| Population circle sizing? | sqrt(pop)/75 (pre-KMZ) | Better visual scaling than linear divisor |
| Population circle colors? | Tiered red/orange/yellow/white | Matches pre-KMZ; legend matches KML |
| GE cache collisions? | Per-scenario legend filenames | Eliminates stale overlay persistence |
| Studio Generator preset? | Yes, with KMZ/title stash | One-click curation for earth system teasers |
| Round 2 separate? | Absorbed into Round 1 | Pre-KMZ restoration fixed all three cards at once |

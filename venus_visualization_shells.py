...
def create_venus_upper_atmosphere_shell(center_position=(0, 0, 0)):
    ...
    layer_info = {
        ...
        'description': (
            ...
            "broadly consider the regions above the main cloud deck (around 70 km) as the upper atmosphere. Here's a look at some key parts of the upper atmosphere <br>" 
            "and their approximate extents:<br>" 
            "* Mesosphere (approximately 60 km to 90-100 km): Above the main cloud layers, the temperature starts to decrease with altitude. This region is considered <br>" 
            ...
            "  Extent in Venus radii: The main part of the ionosphere extends from about 120 km to several hundred kilometers. If we consider an upper limit of, say, 500 km <br>" 
            "  for a significant ionospheric density: So, the ionosphere can extend up to about 8.3% of Venus's radius. However, the outermost fringes can be even more extended.<br>" 
            "* Exosphere (extends from where the atmosphere is very thin outwards into space): The uppermost layer of Venus's atmosphere is the exosphere, where the gas <br>" 
            "  density is so low that atoms and molecules can escape into space. The boundary between the thermosphere and exosphere (the exobase) is not sharply defined but <br>" 
            "  is generally considered to be above where collisions between particles become infrequent. This is likely several hundred kilometers above the surface. The <br>" 
            "  exosphere gradually fades into space and interacts directly with the solar wind. Extent in Venus radii: The exosphere has no well-defined upper limit. It extends <br>" 
            "  outwards until the planet's gravity is no longer the dominant force.<br>" 
...
def create_venus_magnetosphere_shell(center_position=(0, 0, 0), sun_position=(0, 0, 0)):
    ...
    magnetosphere_text = ["Venus has a very weak, induced magnetosphere. Unlike Earth's magnetic field, which is generated internally by its liquid iron core, Venus's <br>" 
            "weak magnetosphere is formed by the interaction of the solar wind with the planet's ionosphere (the upper layer of its atmosphere containing charged <br>" 
            "particles). This induced magnetosphere is not as effective at deflecting charged particles from the Sun as Earth's strong magnetic field.<br><br>" 
            "The solar wind's magnetic field drapes around Venus, creating a comet-shaped magnetosphere with a bow shock on the sunward side and a long magnetotail <br>" 
            "extending away from the Sun. The extent of Venus's induced magnetosphere is highly variable and depends on the strength and direction of the solar wind.<br>" 
            "* Bow Shock: The bow shock, which is the outermost boundary where the supersonic solar wind is slowed and deflected by Venus, typically stands off a few <br>" 
            "  thousand kilometers above the dayside surface. At the subsolar point (the point directly facing the Sun), it's often located around about 1.3 to 1.7 radii <br>" 
            "  from the planet's center. However, this distance can vary significantly with solar wind conditions. The Bow Shock points towards the Sun along the X-axis. <br>" 
            "  The XY plane is the ecliptic.<br>" 
            "* Magnetopause: The magnetopause is the inner boundary of the magnetosphere, where the magnetic pressure of the induced magnetosphere balances the pressure of <br>" 
            "  the shocked solar wind. On the dayside, the magnetopause is typically found much closer to the planet than the bow shock, often around 1.05 to 1.1 radii from <br>" 
            "  the planet's center during solar minimum conditions. It can be pushed closer to the planet during periods of high solar wind pressure.<br>" 
            "* Magnetotail: The magnetotail extends far downstream from Venus, in the anti-sunward direction. Its length is much more variable and can stretch to many Venus <br>" 
            "  radii. Some observations have suggested that the magnetotail can extend to at least ~45 radii and possibly even further during active solar wind conditions.<br>" 
            "  * Estimating the precise base radius and end radius of Venus's induced magnetotail is challenging due to its dynamic nature and dependence on the ever-changing <br>" 
            "    solar wind conditions. Unlike Earth's magnetotail, which is anchored by a strong internal magnetic field, Venus's magnetotail is constantly being shaped and <br>" 
            "    influenced by the solar wind flow and the interplanetary magnetic field (IMF).<br>" 
            "    * Base Radius: The \"base\" of the magnetotail can be considered the region just behind Venus where the induced magnetosphere forms a tail-like structure. <br>" 
            "      This transition region is not a sharp boundary but rather a gradual change in plasma and magnetic field characteristics. Near Venus, the magnetotail <br>" 
            "      has a somewhat cylindrical or lobed structure. Studies using Venus Express data have estimated the typical radius of the near-Venus magnetotail to be <br>" 
            "      around 1 to 1.5 Venus radii.<br>" 
            "    * End Radius (Length): The length of Venus's magnetotail is highly variable and can extend to significant distances downstream. Recent flybys by spacecraft <br>" 
            "      like Solar Orbiter have provided new insights into the far reaches of the tail. Observations have detected the presence of the magnetotail and its <br>" 
            "      boundaries (bow shock and induced magnetospheric boundary) extending to at least 60 Venus radii (~363,120 km) downstream. There is even some evidence <br>" 
            "      suggesting that under certain solar wind conditions, the tail might extend even further, possibly up to or beyond 100 Venus radii. One study of Mariner 10 <br>" 
            "      data suggested possible detection of the tail as far as ~100 radii downstream. <br>" 
            "      * Dynamic Nature: The solar wind's pressure, speed, and the orientation of the IMF constantly buffet Venus's induced magnetosphere, causing significant <br>" 
            "        variations in its size and shape.<br>" 
            "      * Boundary Identification: Defining the exact outer boundary of the magnetotail can be challenging as the plasma environment gradually transitions back to <br>" 
            "        the solar wind.<br>" 
            "      * Limited Far-Tail Observations: While recent missions have provided valuable data, the far magnetotail of Venus has not been as extensively sampled as <br>" 
            "        Earth's.<br>" 
            "    * Estimating the width of Venus's magnetotail at distances of 45 to 60 Venus radii is challenging due to the limited number of direct observations so far out <br>" 
            "      in the tail. Most detailed studies have focused on the near-Venus magnetotail (within ~10 to 12 radii). However, recent flybys, particularly by Solar <br>" 
            "      Orbiter, are providing new insights.<br>" 
            "      * Overall Expansion: The induced magnetotail of Venus is known to flare or expand as it extends downstream from the planet. This is a common feature of <br>" 
            "        magnetotails formed by the interaction of the solar wind with a planetary obstacle.<br>" 
            "      * Solar Orbiter Observations: A recent study based on Solar Orbiter flybys, which reached as far as 60 radii downstream, suggests that the bow shock (the <br>" 
            "        outer boundary) at this distance is about 15-20 radii in the lateral direction (which would give a total width of roughly 30-40 radii if we assume some <br>" 
            "        symmetry around the tail axis). The induced magnetospheric boundary (IMB), which is inside the bow shock, was still detected at 20 radii downstream in <br>" 
            "        this study, but its width at larger distances was not specifically quantified.<br>" 
            "      * Earlier Estimates: Older studies based on Pioneer Venus Orbiter data, which primarily sampled the tail up to ~12 radii, found the tail to be about 4 <br>" 
            "        radii wide and 3.2 radii high at that distance. Extrapolating this far out is difficult due to the flaring. Therefore, a reasonable estimate for the <br>" 
            "        width of Venus's magnetotail at 45 to 60 Venus radii would likely be on the order of tens of Venus radii, possibly in the range of 20 to 40 radii or <br>" 
            "        even wider, based on the observed bow shock dimensions at that distance. The actual width can vary significantly depending on the solar wind conditions <br>" 
            "        (pressure, speed, IMF orientation). The shape of the tail might not be perfectly circular at these large distances.<br>"
            "In summary, the extent of Venus's induced magnetosphere in terms of Venus radii is:<br>" 
            "* Dayside (from the center): Extends to roughly 1.3 - 1.7 radii at the bow shock and about 1.05 - 1.1 radii at the magnetopause.<br>"
            "  The Bow Shock points towards the Sun along the X-axis. The XY plane is the ecliptic.<br>" 
            "* Nightside (magnetotail): Can extend to at least tens of Venus radii, with estimates reaching up to ~45 radii or more under certain solar wind conditions.<br>" 
            "The size and shape of Venus's induced magnetosphere are highly dynamic, constantly being shaped by the ever-changing solar wind."]

...
radiation_belts`) but are deliberately excluded here, and the reasoning
below corrects an earlier draft of this section (v1/early v2 read the
`# Source: NASA Jupiter Ring Fact Sheet` comment above `ring_params` as
covering the whole dict per the project's usual "unit of provenance"
convention, and treated the colors as citable on that basis -- that was
wrong). Tony's direct call (July 16, 2026): that citation was only ever
verified for the ring geometry: the color values are developer/AI
aesthetic picks for visual distinction, never checked against the cited
source or any other photometric measurement, and the source itself now
carries an in-line note narrowing the citation's scope to geometry only
[jupiter_visualization_shells.py, `create_jupiter_ring_system`, edited
July 16, 2026]. Serving them as part of a `features` schema would imply a
confidence they don't have. Jupiter's radiation-belt colors were already
excluded for lack of any adjacent citation at all -- same outcome, now
the same reason on both counts. A real color-accuracy pass is tracked as
a low-priority wishlist item, LEDGER_CONSOLIDATED.md L-124, with NASA/
Cassini sourcing captured there for whoever picks it up -- not this
manifest's scope.)
...
fields, none of which are ported. Saturn's ring colors never had any
adjacent citation at all (checked directly; the nearest `# Source:`
comment in that file is about the cloud-layer atmosphere, not the rings),
so this one was never a close call the way Jupiter's was. Same reasoning
as Jupiter above applies regardless: these are developer/AI aesthetic
picks, not measured values, now disclosed as such in-source
[saturn_visualization_shells.py, `create_saturn_ring_system`, edited
July 16, 2026], and tracked as the same L-124 wishlist item rather than
served here.)

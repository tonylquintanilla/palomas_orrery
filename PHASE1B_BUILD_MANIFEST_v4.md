...
- **`Pluto_Sun` is the Pluto BODY (id 999).** `celestial_objects.py` defines
  `Pluto` id `999` and a separate `Pluto-Charon Barycenter` id `9`. So
  `pluto.json` from `Pluto_Sun` is the body's heliocentric path; it is
  labeled honestly as the body (the `trajectory_of: pluto_barycenter`
  aspiration is dropped until the Pluto-Charon subsystem is served).
- **Direct relative-frame pairs exist** (the correct source, no subtraction):
  `Charon_Pluto-Charon Barycenter` (564 pts, 2025-11-24..2027-06-10),
  `Moon_Earth-Moon Barycenter` (494 pts), plus the full Pluto system
  ...
  -- a Phase 2 coverage limit, not a Phase 1b blocker.
- **Osculating cache healthy:** 115 entries; 9/9 tranche present; `Charon@9`
  center `@9`; **8/9 epochs carry HH:MM**; 0/9 `MA=None`.
...
<output_dir>/
  ...
  positions/
    ...
    pluto.json             # heliocentric, km -- the Pluto BODY (id 999),
                           #   labeled as body, not barycenter
    voyager_1.json         # arc-natural, km
    moon.json              # center = moon's osculating center; ~27 pts/orbit
    titan.json             # center = saturn; ~16 pts/orbit
    (charon.json)          # center = pluto_barycenter; ~6.4 pts/orbit
                           #   -- MARGINAL; ships only if Mode 5 accepts it
...
`position_pair_key` examples: Charon -> `Charon_Pluto-Charon Barycenter`;
Moon -> the pair matching its osculating center (confirm `@399` body vs
Earth-Moon-barycenter at build -- do NOT assume). Io -> None (no usable
trace). CENTER_SLUG_MAP extended to the barycenter center names actually
present as pair-key centers.
...
## 8. Open decisions carried into the build session
- **Charon in or out** of the daily tranche (6.4 pts/orbit hexagon) -- Mode 5
  render call. Its orbit ships either way; this is only about the trace file.
- **Moon frame:** `@399` body vs Earth-Moon barycenter for the served trace
  -- confirm the pair whose center matches Moon's osculating (`@399`) at build;
  do not assume.
- **Pluto-Charon relative subsystem** (Charon/Styx/Nix/Kerberos/Hydra fine
  cadence + the 29-pt barycenter heliocentric entry) -- deferred to a fine-
  cadence follow-on; tracked, not built here.
...
grounded against idealized_orbits.py at HEAD).

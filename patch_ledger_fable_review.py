# -*- coding: utf-8 -*-
"""patch_ledger_fable_review.py -- record the Fable design review of the
feature/constant unification architecture, and close out the three items
...
- **RECOMMENDED (not yet ruled): vendored pull.** The builder does
  `git ls-remote` on the orrery, records the HEAD SHA, fetches the
  generated feature artifact by raw URL AT THAT SHA (not at `main`
  blind), validates shape + source-presence + content hash, then commits
  it with the rest of the nightly output under existing atomic-swap /
  quarantine semantics. On unreachable-or-invalid: quarantine, keep
  serving the last committed copy, warn loudly.
...
- Tony-action (decide): ratify or reject vendored pull.
- Tony-action (decide): L-179 and L-180 values, as the gate above.
"""
...
EDITS = [
    ...
    ('F-3a', 'L-179: gates the L-181 migration',
     ...
     b"- **GATES L-181** (Fable review, 2026-08-06). Migrating and deriving\n"
     b"  before this value is settled would transport a known-inconsistent\n"
     b"  number into the gallery artifact, the served cache, and the hover\n"
     b"  text -- authoritative-looking in three more places. Settle first.\n"
     b"**Gap:** Resolve which value is authoritative. Update the loser.\n"
     b"**Ref:** FABLE_shell_consistency_audit_report.md findings #29-30;\n"
    ...
    ('F-3b', 'L-180: gates the L-181 migration',
     ...
     b"- **GATES L-181** (Fable review, 2026-08-06). Same reasoning as L-179:\n"
     b"  do not transport an unsettled value into three more places.\n"
     b"**Gap:** Reconcile text, add Show-the-Envelope comment.\n"
     b"**Ref:** FABLE_shell_consistency_audit_report.md finding #31;\n"
     b"FABLE_REVIEW_feature_constant_unification.md sequencing note."),
...
def main():
    ...
    print("")
    print("STILL OPEN (recorded as decisions, NOT as rulings):")
    print("  - vendored pull: ratify or reject          [L-181]")
    print("  - L-179 / L-180 values, gating migration   [L-179/L-180]")
    print("  - annotation parser: strip or extend       [L-186]")
    return 0

...
## Tony with Claude | v3.4 | December 24, 2025
...
### Agentic Pre-Test Protocol (NEW in v3.4)
...
# 6. Final encoding prep
sed -i 's/$/\r/' palomas_orrery.py  # CRLF for Windows
```
...
### File Encoding for Windows

When generating files for Tony's Windows environment:
...
file filename.py                     # Check line endings (should show CRLF)
```

**If delivering a full file:** Convert line endings with `sed -i 's/$/\r/' filename`
...
*~550 lines. Functional for Claude, readable for human, signal preserved.*

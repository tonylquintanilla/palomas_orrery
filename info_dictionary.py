...
Contains hover text descriptions, object type mappings, luminosity class
definitions, and GUI note text. These are PUBLIC-FACING strings displayed
in the GUI, hover text, and gallery -- factual claims in these strings
should be verified against authoritative sources.

Split from constants_new.py to separate narrative content (fact-checked)
from numeric constants (source-cited). See provenance_scanner.py for audit.

Key dictionaries:
    INFO - Object descriptions shown in GUI info panel and hover text
    ...
    note_text - GUI welcome/about text

Consumed by: palomas_orrery, palomas_orrery_helpers, gallery_studio,

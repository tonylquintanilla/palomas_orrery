...
def get_encounter_preset(spacecraft_name, encounter_index):
    ...
    #    plot_scale = enc.get('plot_scale_au') or resolution['plot_scale_au']
        ...
        # Fallback: manual values from dict (whole days or fractional)
        plot_days = enc.get('plot_days_closeup', enc.get('plot_days', 28))
        plot_scale = enc.get('plot_scale_au_closeup', enc.get('plot_scale_au'))
        # Adjust fetch_step for sub-day windows
        if plot_days < 1:
            fetch_step = '1m'   # Sub-day: 1-minute resolution
        elif plot_days <= 3:
            fetch_step = '1h'
        else:
            fetch_step = '6h'
        ...
        enc_date = _parse_encounter_date(enc.get('date', ''))
        if enc_date:
            half_window = timedelta(days=plot_days / 2)
            start_dt = enc_date - half_window
            end_dt = enc_date + half_window
...
def add_tagged_encounter_marker(fig, encounter, position, spacecraft_name, color_map):
    ...
    fig.add_trace(
        go.Scatter3d(
            ...
            name=f"{spacecraft_name} {label}" + (" (Horizons)" if encounter.get('_horizons_refined') else ""),
            text=[hover_text],

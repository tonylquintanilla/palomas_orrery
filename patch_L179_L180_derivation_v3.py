...
def main():
    ...
    for name, fp_expected in FINGERPRINTS.items():
        ...
        # can hold CRLF where the repo holds LF (.gitattributes sets eol=lf,
        # so git normalizes on commit and reports no change). Comparing raw
        # bytes would call that "base moved" when nothing was edited.
        ...
        if fp_actual != fp_expected:
            problems.append(
                ...
                f"    (line endings already normalized, so this is a real\n"
                f"     content difference.) Do not force it."
            )
            continue

        # Match and write in whatever convention this file already uses, so a
        # CRLF file stays CRLF and an LF file stays LF. Anchors below are LF.
